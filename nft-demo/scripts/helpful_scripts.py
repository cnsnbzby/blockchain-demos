...
OPENSEA_URL = "https://testnets.opensea.io/assets/{}/{}"  # here first bracket is for contract address and second is tokenid
BREED_MAPPING = {0: "PUG", 1: "SHIBA_INU", 2: "ST_BERNARD"}  # breed dictionary
...
# in order to interact with real onchain or mock offchain versions of contracts we need below function
# below function is smart enough if we deploy a mock or grab from actual contract
...
def fund_with_link(
    ...
    # account will be equal to account if account is supplied, else (otherwise) use get_account
    link_token = link_token if link_token else get_contract("link_token")
    funding_tx = link_token.transfer(contract_address, amount, {"from": account})
    # above line is transfering certain amount to contract address
